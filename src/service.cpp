...
std::string service::formattedString(AMX* amx, cell* params, cell text_index, int32_t args_offset)
{
	...
	// Arg�man say�s� offset say�s�na ula��yor ve ge�iyorsa formatlama a�amas�na ge�
	if (max_args >= args_offset)
	{
		// Metini tarayacak index de�erimiz
		size_t index = 0;

		// Arg�man�n ba�lang�� offsetini ayarla
		int args_count = args_offset;

		// D�ng� ile metinleri tara
		while (index < metin.length())
		{
			// Taranan metinde % karakteri var m�?
			if (metin[index] == '%')
			{
				// Varsa index de�erini +1 olarak artt�r
				index++;

				// Taranan % karakteri var m� kontrol et, varsa karakteri % olarak ayarla ve bu indexi ge�
				if (metin[index] == '%')
				{
					metin[index] = '%';
					continue;
