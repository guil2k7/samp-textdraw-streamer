...
cell AMX_NATIVE_CALL Natives::DynamicTextDrawShowForPlayer(AMX* amx, cell* params)
{
	...
	// G�sterilecek oyuncu sunucuda yoksa alt fonksiyonlar� �al��t�rma
...
cell AMX_NATIVE_CALL Natives::DynamicTextDrawShowForAll(AMX* amx, cell* params)
{
	...
	// Sunucuda bir oyuncu yoksa bu fonksiyonu �al��t�rmay� durdur
	...
	// Aktif g�z�ken bir kullan�c� yoksa textdraw� yeniden olu�tur
	if (GlobalText::gTextVisible[textid].empty())
	{
		// TextDraw'� olu�tur
		GlobalText::Reload(it);

		// TextDraw olu�turulduysa
		if (it->second->real_id != INVALID_DYNAMIC_PLAYER_TEXTDRAW)
		{
			// T�m oyuncular� g�z�k�yor olarak ayarla
	...
	{
		// Zaten g�z�ken oyuncu oldu�u i�in ger�ek kimlik ile tekrar g�ster
		TextDrawShowForAll(it->second->real_id);
	}
...
//
// native DynamicTextDrawSetStringFor(Text:textid, playerid, const format[], {Float, _}:...);
//
cell AMX_NATIVE_CALL Natives::DynamicTextDrawSetStringForPlayer(AMX* amx, cell* params)
{
	CHECK_PARAMS(params[0] / sizeof(cell));
	...
	if (it->second->real_id != INVALID_DYNAMIC_PLAYER_TEXTDRAW)
	{
		TextDrawSetStringForPlayer(
			it->second->real_id,
			playerid,
			service::formattedString(amx, params, 3, 4).c_str()
		);
	}
